...
class TestSambanisRF(TestCase):
    ...
    def test_with_good_params(self):
        ...
        with TemporaryDirectory() as td:
            build([SambanisRFCV(output_path=td, model_params={"n_estimators": 100})], local_scheduler=True)
    ...
    def test_with_bad_params(self):
        ...
        with TemporaryDirectory() as td:
            build([SambanisRFCV(output_path=td, model_params={"n_estimators": 1})], local_scheduler=True)
            with open(os.path.join(td, "auc.txt")) as f:
                result = float(f.read())
        print(result)
